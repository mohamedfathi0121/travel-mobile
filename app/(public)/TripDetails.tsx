...
const handleSave = async () => {
    const totalCapacity = (singleCount * 1) + (doubleCount * 2) + (tripleCount * 3); // حساب السعة الإجمالية للغرف
    ...
    // ✅ تحديث منطق التحقق من عدد الأشخاص مقابل سعة الغرف
    if (peopleCount > totalCapacity) {
      setModalMessage(
        `You selected ${peopleCount} people, but the chosen rooms can only accommodate ${totalCapacity}.\nPlease add more rooms or adjust the number of people.`
      );
      return setShowModal(true);
    }
